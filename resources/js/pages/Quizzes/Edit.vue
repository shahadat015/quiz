                ...
                <form @submit.prevent="updateQuestion" class="space-y-4">
                    <div class="sm:flex space-y-4 sm:space-y-0 sm:space-x-6">
                        ...
                        <div class="sm:w-1/2 space-y-4">
                            ...
                            <app-button classes="w-full" :disabled="loading">Update</app-button>
                        </div>
                    </div>
                </form>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            async updateQuestion() {
                try {
                    this.loading = true;
                    let { data } = await axios.put(`/api/quizzes/${this.$route.params.id}`, this.form);
                    this.loading = false;
             ...
             async getQuestion() {
                try {
                    this.loading = true;
                    let { data } = await axios.get(`/api/quizzes/${this.$route.params.id}`);
                    this.form = data.data;
                    this.loading = false;
                } catch (error){
                    this.loading = false;
